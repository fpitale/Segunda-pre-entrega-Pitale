...
class Cliente:
    def __init__(self, nome, sobrenome, email, idade):
        self.nome = nome
    ...
    def __str__(self):
        return f"{self.nome} {self.sobrenome}, {self.idade} anos - Email: {self.email}"
...
def login():
    while True:
        # Solicitar nome de usuário para iniciar sessão
...
def main():
    ...
    while True:
        ...
        if option == "1":
            ...
            nome = input("\nIngrese su nombre: ")
            sobrenome = input("Ingrese su apellido: ")
            email = input("Ingrese su email: ")
            idade = input("Ingrese su edad: ")

            cliente = Cliente(nome, sobrenome, email, idade)

            if cliente.verificar_email():
